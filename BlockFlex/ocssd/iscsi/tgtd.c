...
static struct option const long_options[] = {
	...
	{"pfe-err-blk", required_argument, 0, 'e'},
	{"pfe-io-header-log", required_argument, 0, 'a'},
	{"pfe-io-data-log", required_argument, 0, 'b'},
	{"pfe-fail-type-tgtd", required_argument, 0, 'n'},
	{"pfe-enable-record", required_argument, 0, 'r'},
	{0, 0, 0, 0},
};
...
static void usage(int status)
{
	...
	printf("Linux SCSI Target framework daemon, version %s\n\n"
        ...
        "-e, --pfe-err-blk             log file storing emulated io err block#s\n"
        "-a, --pfe-io-header-log       log file storing io header\n"
        "-b, --pfe-io-data-log         log file storing io data\n"
        "-n, --pfe-fail-type-tgtd      fail type to emulate in tgtd\n"
        "-r, --pfe-enable-record       enable recording write requests in tgtd",
...
int main(int argc, char **argv)
{
    ...
    //////pfe log files
    char * pfe_err_blk_fname = NULL;
    char * pfe_io_header_fname = NULL;
    char * pfe_io_data_fname = NULL;
    ////
	...
	while ((ch = getopt_long(argc, argv, short_options, long_options,
		...
		case 't':
			...
			break;
		case 'd':
			ret = str_to_int_range(optarg, is_debug, 0, 1); //is_debug range in [0, 1]
            //printf("PTE: ret = %d\n ", ret);
            //printf("PTE: is_debug = %d\n ", is_debug);
			if (ret)
				bad_optarg(ret, ch, optarg);
			break;
        case 'a'://PTE io header log
            pfe_io_header_fname = optarg;
            break;
        case 'b'://PTE io data log
            pfe_io_data_fname = optarg;
            break;
        case 'e'://PTE log with error blcok#s (should ret = -1 errno = 5)
            pfe_err_blk_fname = optarg;
            break;
        case 'n'://PTE fail type to emulate in tgtd (ret err on read)
            pfe_fail_type_tgtd = strtol(optarg, NULL, 0);
            break;
        case 'r'://enable recording io 
            pfe_enable_record = strtol(optarg, NULL, 0);
            break;
		case 'V':
			version();
			break;
    ...
    pfe_io_header_fd = open(pfe_io_header_fname, O_CREAT | O_TRUNC | O_WRONLY | O_SYNC, 0666 );
    if(pfe_io_header_fd < 0){
        printf("PFE: ERROR in opening %s !!!\n", pfe_io_header_fname);
        return -1; 
    }
    //printf("PFE: opened %s\n", pfe_io_header_fname);

    //init first header in io header log (64 B) (to store total io cnt)
    char * super_head = calloc(sizeof(pfe_io_header_t), 1);
    ret = pwrite(pfe_io_header_fd, super_head, sizeof(pfe_io_header_t), 0);
    if(ret < 0){
        printf("PFE: init sup_head failed! ret = %d \n", ret);
        return ret;
    }

    pfe_io_data_fd = open(pfe_io_data_fname, O_CREAT | O_TRUNC | O_WRONLY | O_SYNC, 0666);
    if(pfe_io_data_fd < 0){
        printf("PFE: ERROR in opening %s !!!\n", pfe_io_data_fname);
        return -1; 
    }
    ...
    if(pfe_fail_type_tgtd == 5 || pfe_fail_type_tgtd == 7 ||
       pfe_fail_type_tgtd == 8 ) {

        pfe_err_blk_fd = open(pfe_err_blk_fname, O_CREAT | O_RDONLY, 0666 );
        if(pfe_err_blk_fd < 0){
            printf("PFE: ERROR in opening %s !!!\n", pfe_err_blk_fname);
            return -1; 
        }

        pfe_build_errblks_cache(pfe_err_blk_fd);
    ...
    ///////////////
    close(pfe_io_header_fd);
    close(pfe_io_data_fd);

    if(pfe_fail_type_tgtd == 5 || pfe_fail_type_tgtd == 7 ||
       pfe_fail_type_tgtd == 8 ) {

        pfe_free_errblks_cache();
        close(pfe_err_blk_fd);
    }
    ///////////////
