...
def train_online(model, optimizer, train_sequence, train_bool, epochs=15, batch_size=1):
...
def preprocessing(input_data, label_data, win):
    ...
    for i in range(L-win):
        ...
        if (train_label[0] < outputs): train_label[0] = 0
...
def read_input(in_file):
    ...
    input_data = [[] for _ in range(channels)]
    ...
    with open(in_file, "r") as f:
        for line in f:
            ...
            #input_data.append([float(temp[0])/time_alloc, int(float(temp[1]))])
            ...
            for i in range(channels):
                ...
                if i < curbw:
                    if hist[i] > 0:
                        ...
                        while cur <= duration:
                            input_data[i].append([cur/time_alloc])
                            ...
                            #Every five minutes
                            cur += rem
                        #input_data[i].append([duration/time_alloc])
                    ...
                    while hist[i]-max_hist[i] > out_lim:
                        input_data[i].append([max_hist[i]/time_alloc])
                        train_bool[i].append(True)
...
if mode == "train":
    ...
    #for i in range(5):
        print(f"Training: {i}")
        pred_online = train_online(model[i], optimizer[i], input_seq[i], train_bool[i], epochs=epochs, batch_size=batch_size)
...
#    torch.save(checkpoint, in_file + str(i)+'_dur_sz.model')
